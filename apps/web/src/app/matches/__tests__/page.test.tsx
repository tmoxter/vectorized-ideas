...
describe("MatchesPage Integration Tests", () => {
  ...
  beforeEach(() => {
    ...
    mockSupabaseClient.mockReturnValue(
      createMockSupabaseClient(currentUser.id)
    );
  ...
  it("should show message when no candidates are found", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/no potential matches found/i)
      ).toBeInTheDocument();
    });
  ...
  it("should display error message when API call fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/error loading matches: server error/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("should show end message after reviewing all candidates", async () => {
    ...
    // First candidate
    const connectButton1 = screen.getByRole("button", {
      name: /let's connect/i,
    });
    ...
    // Second candidate
    const connectButton2 = screen.getByRole("button", {
      name: /let's connect/i,
    });
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(mockCandidates[2].profile.name)
        ).toBeInTheDocument();
      },
      { timeout: 2000 }
    );
    ...
    const connectButton3 = screen.getByRole("button", {
      name: /let's connect/i,
    });
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/you've reviewed all available matches/i)
        ).toBeInTheDocument();
      },
      { timeout: 2000 }
    );
