...
describe('LandingPage', () => {
  ...
  it('should handle successful signup', async () => {
    ...
    await user.type(screen.getByPlaceholderText(/password \(min 6 characters\)/i), testPassword);
  ...
  it('should expand and collapse FAQ items', async () => {
    ...
    const faqButton = screen.getByText(/Aren't there already alternative platforms out there\?/i);

    // FAQ should not be expanded initially
    expect(screen.queryByText(/Yes, but I found it hard to find people/i)).not.toBeInTheDocument();

    // Click to expand
    await user.click(faqButton);

    // FAQ content should be visible
    expect(screen.getByText(/Yes, but I found it hard to find people/i)).toBeInTheDocument();
